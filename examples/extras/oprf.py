...
from extras.parameters import sigma_max
...
def server_prf_online_single_thread(key, vector_of_pairs):
    '''
    :param key: an integer
    :param vector_of_pairs: vector of coordinates of some points P on the elliptic curve
    :return: vector of coordinates of points key * P on the elliptic curve
    '''
    # Convert the pairs of coordinates into elliptic curve Points
    vector_of_points = [Point(P[0], P[1], curve=curve_used) for P in vector_of_pairs]

    # Compute key * P for each point P in vector_of_points sequentially
    vector_of_multiples = [key * P for P in vector_of_points]

    return vector_of_multiples
